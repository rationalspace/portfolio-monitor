"""Tests for the Broker → Ghostfolio CSV converter.
...
class TestClassifyAction:
    ...
    def test_you_bought_verbose(self):
        assert classify_action("YOU BOUGHT BROKER GOVT MONEY MARKET") == "BUY"
...
def _write_broker_csv(path: Path, rows: list[dict]) -> None:
    """Write a minimal Broker-style CSV (with preamble) to path."""
    fieldnames = [
    ...
    with path.open("w", newline="") as f:
        # Broker exports start with junk before the header.
        f.write("Brokerage,Broker\n")
...
class TestConvert:
    def test_single_buy(self, tmp_path):
        inp = tmp_path / "broker.csv"
        out = tmp_path / "ghostfolio.csv"
        _write_broker_csv(inp, [
            {
                "Run Date": "04/15/2024",
                "Action": "YOU BOUGHT",
                "Symbol": "GOOGL",
