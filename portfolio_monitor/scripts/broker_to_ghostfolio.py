"""Convert a Broker Activity CSV into Ghostfolio's import format.

Broker export → Ghostfolio CSV mapping:

| Broker column          | Ghostfolio column | Notes                                     |
|--------------------------|-------------------|-------------------------------------------|
...
Skipped Broker action types (cash movements, transfers, etc.) are reported
...
_NON_EQUITY_SYMBOLS: frozenset[str] = frozenset({
    "SPAXX", "FDRXX", "FZFXX", "FMPXX",  # Broker money-market funds
    "FCASH", "CORE",                        # Cash core positions
})

# Broker action strings → Ghostfolio activity type. Anything not listed is skipped.
...
def classify_action(raw: str | None) -> str | None:
    """Find the first matching action keyword in the Broker 'Action' string.

    Broker's Action field is verbose ("YOU BOUGHT BROKER GOVT CASH...").
...
def normalize_symbol(raw: str | None) -> str | None:
    ...
    if not sym:
        return None
    # Skip Broker money-market codes (suffix '**' indicates a core position).
    if sym.endswith("**"):
        return None
...
def reformat_date(raw: str | None) -> str | None:
    """Broker uses MM/DD/YYYY; Ghostfolio wants YYYY-MM-DD."""
    raw = (raw or "").strip()
...
def parse_money(raw: str | None) -> float:
    """Broker values can be '$1,234.56' or '($1,234.56)' or blank."""
    if not raw:
        return 0.0
...
def convert(
    ...
    # Find the header row — Broker sometimes adds a preamble, and always
    ...
    if header_idx is None:
        raise SystemExit("Could not find Broker header row (looking for 'Run Date', 'Action', 'Symbol').")
    ...
    for row in reader:
        ...
        # Real Broker exports use "Price ($)" — fall back to "Price" for test fixtures.
        ...
        ghostfolio_acct = account_map.get(broker_acct, default_account)
        if ghostfolio_acct is None:
            skipped += 1
            skip_reasons[f"unmapped account ({broker_acct or 'unknown'})"] += 1
            continue
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Convert a Broker Activity CSV to Ghostfolio import format.")
    parser.add_argument("input", type=Path, help="Path to the Broker Accounts_History CSV.")
    parser.add_argument("--output", type=Path, default=Path("ghostfolio_import.csv"))
    parser.add_argument(
        "--account-map",
        action="append",
        default=[],
        metavar="BROKER_ID=GHOSTFOLIO_UUID",
        help="Map a Broker account ID to a Ghostfolio account UUID. Repeatable.",
    )
    parser.add_argument(
        "--default-account",
        default=None,
        help="Ghostfolio account UUID for any rows whose Broker ID isn't mapped.",
    )
    ...
    for spec in args.account_map:
        if "=" not in spec:
            parser.error(f"--account-map needs BROKER_ID=GHOSTFOLIO_UUID, got: {spec}")
        k, v = spec.split("=", 1)
