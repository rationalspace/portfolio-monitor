...
SnapTrade Fidelity is read-only anyway), additional methods can be added.
"""
...
_NON_EQUITY_SYMBOLS: frozenset[str] = frozenset({
    ...
    "FCASH",   # Fidelity cash core position
    "FMPXX",   # Fidelity Prime Money Market
...
class SnapTradeClient:
    ...
    def fetch_portfolio(self) -> Portfolio:
        """Pull current holdings + transactions for every linked Fidelity account.
    ...
    def _account_transactions(self, account_id: str) -> list[dict[str, Any]]:
        ...
        SnapTrade. Import Fidelity's own transaction CSV into Ghostfolio
        instead (see scripts/fidelity_to_ghostfolio.py) for accurate lot dates.
        """
        try:
